...
static int fts_test_entry(char *ini_file_name)
{
    ...
    if (true == fts_test_start()) {
        FTS_TEST_SAVE_INFO("\n\n=======Tp test pass.\n");
        ret = 1;//bug:446258,liutongxing.wt,add,20190521,tp openshort test.
    } else {
        FTS_TEST_SAVE_INFO("\n\n=======Tp test failure.\n");
        ret = 0;//bug:446258,liutongxing.wt,add,20190521,tp openshort test.
    }

    //ret = 0; //bug:446258,liutongxing.wt,remove,20190521,tp openshort test.
test_err:
    fts_test_main_exit();
    enter_work_mode();
    return ret;
