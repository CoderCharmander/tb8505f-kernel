...
static int ft8613s_open_test(struct fts_test *tdata, bool *test_result)
{
    ...
    for (i = 0; i < FACTORY_TEST_RETRY; i++) {
        ...
        if ((ret >= 0) && (FACTORY_REG_OPEN_IDLE == state)) {
            break;
        } else if ((ret >= 0) && (FACTORY_REG_OPEN_BUSY == state)) {
            sys_delay(FACTORY_TEST_RETRY_DELAY);
        } else if ((ret >= 0) && (TEST_RETVAL_AA == state)) {
            ret = fts_test_write_reg(FACTORY_REG_OPEN_STATE,
                                     FACTORY_REG_OPEN_IDLE);
            if (ret < 0) {
